  ...
  try {
    for (const record of event.Records) {
      ...
      try {
        ...
        body = JSON.parse(rawBody.Message);
      } catch (parseError) {
        console.log("Erro ao parsear a mensagem", parseError);
        throw new Error("Erro ao parsear a mensagem do SQS");
      }

      const ownerId = body.ownerId;
      const bucketName = "notes-catalog-marketplace";
    ...
    console.log("Erro ao processar mensagem do SQS", error);
...
async function getS3Object(bucket, key) {
  ...
  try {
    ...
    console.log("Erro ao obter objeto do bucket", error);
...
async function putS3Object(dstBucket, dstKey, content) {
  ...
  try {
    ...
    console.log("Erro ao colocar objeto no bucket", error);
    throw new Error('Error putting object in bucket');
  }
}
